...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns) {
    ...
    if (Array.isArray(obj)) {
        ...
        for (var name in obj) {
            var child = obj[name];
            var trackData = false;
            if(name.indexOf("TrackData") >= 0){
              trackData = true;
//              console.log("Got TrackData :"+name);
              var method = name.replace("TrackData","");
              name = "TrackData";
              var priorxmlnsAttrib = xmlnsAttrib;
              if(method == "swipe" || method == "proximity"){
                xmlnsAttrib = ' method="'+method+'"';
              }
            }
            parts.push(['<',ns,name,xmlnsAttrib,'>'].join(''));
            if(trackData){
              xmlnsAttrib = priorxmlnsAttrib;
            }
